...
function AllProducts() {
  ...
  const fetchProducts = async (filters = {}) => {
    try {
      ...
      let query = `https://bonntonn.up.railway.app/api/v1/products/filter-products?priceRange=${priceRange || '100-9999'}&productName=${productName || ''}`;
      if (catagory) {
  ...
  const getCatagoryList = async () => {
    try {
      const response = await fetch('https://bonntonn.up.railway.app/api/v1/products/get-catagories', { method: 'GET' });
      if (response.ok) {
  ...
  const productDelete = async () => {
    try {
      const request = await fetch(`https://bonntonn.up.railway.app/api/v1/products/delete-product?productId=${deleteProduct}`, { method: 'DELETE' });
      if (request.ok) {
